...
function o.init()
  ...
  settings.define( "storage", { description = "The configured storages carried by the turtle.", default = {}, type = "table" } )
  settings.define( "refuel_all", { description = "If the turtle should consume the whole stack of fuel.", default = false, type = "table" } )
...
function o.save_inventory()
  turtle.data.slots = turtle.get_slots()
  o.set( "slots", turtle.data.slots )
end

function o.save_facing()
  o.set( "facing", { dx = turtle.data.dx, dz = turtle.data.dz } )
end

function o.save_position()
  o.set( "position", vector.new( turtle.data.x, turtle.data.y, turtle.data.z ) )
end

-- Set the values.
function o.set_forbidden_block( new_forbidden_block )
  turtle.forbidden_block = new_forbidden_block
  settings.set( "forbidden_block", turtle.forbidden_block )
  settings.save( FILE_NAME )
end

function o.set_valid_fuel( new_valid_fuel )
...
-- Remove any storage settings.
function o.remove_storage_config( index )
